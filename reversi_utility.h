...
static bool has_player_move(int board[N][N], int id, Move start, int i_delta, int j_delta) {
...
/* Return all the possible valid moves on a board for a speific player(id) */
static std::vector<Move> possible_moves(int board[N][N], int id) {
	...
	for (int i = 0; i < N; i++) {
		for (int j = 0; j < N; j++) {
			if (board[i][j] == 0) {
				bool legal = (i < N - 1 && board[i+1][j] == opponent_id && has_player_move(board, id, Move{i+1, j}, 1, 0)) || 
							 (i >= 1 && board[i-1][j] == opponent_id && has_player_move(board, id, Move{i-1,j}, -1, 0)) || 
							 (j < N - 1 && board[i][j+1] == opponent_id && has_player_move(board, id, Move{i,j+1}, 0, 1)) || 
							 (j >= 1 && board[i][j-1] == opponent_id && has_player_move(board, id, Move{i,j-1}, 0, -1)) || 
							 (i < N - 1 && j < N - 1 && board[i+1][j+1] == opponent_id && has_player_move(board, id, Move{i+1, j+1}, 1, 1)) ||
							 (i >= 1 && j >= 1 && board[i-1][j-1] == opponent_id && has_player_move(board, id, Move{i-1, j-1}, -1, -1)) || 
							 (i >= 1 && j < N - 1 && board[i-1][j+1] == opponent_id && has_player_move(board, id, Move{i-1, j+1}, -1, 1)) || 
							 (i < N - 1 && j >= 1 && board[i+1][j-1] == opponent_id && has_player_move(board, id, Move{i+1, j-1}, 1, -1));
				if (legal) {
...
/* Return winners based on the given board state */
...
/* Update the board with given move of a specific player */
static std::vector<Move> update(int board[N][N], Move move, int id) {
	...
	deltas.push_back(std::make_pair(-1,-1));
	deltas.push_back(std::make_pair(1,1));
	/* Flip the opponent's move */
	for (const auto& delta : deltas) {
		if (move.i < N && move.i >= 0 && move.j < N && move.j >= 0 && 
			has_player_move(board, id, Move{move.i+delta.first, move.j+delta.second}, delta.first, delta.second)) {
